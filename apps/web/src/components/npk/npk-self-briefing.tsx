...
import { TOTAL_ASPEK, WIRED_ASPEK, zoneOf } from "./npk-status";
      ...
      <div className="space-y-3 px-5 py-4">
        <p className="text-sm leading-relaxed text-white/90">
          {!hasData ? (
            ...
            <>
              NPK Anda <span className="font-semibold text-white">{fmt1(data.npk)}/100</span> — {zone.label} ({PREDIKAT_LABEL[data.predikat]}), {data.available_count}/{TOTAL_ASPEK} aspek terukur.
        ...
        {provisional && (
          <p className="flex items-start gap-1.5 rounded-lg bg-white/10 px-3 py-2 text-xs text-white/85">
            ...
            Skor SEMENTARA — baru <span className="font-semibold">{WIRED_ASPEK}/{TOTAL_ASPEK} aspek</span> (Revenue, AR) yang punya feed data live. Aspek lain menyusul.
          </p>
        )}

        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          <Tile label="NPK"><span className="text-xl font-bold tabular-nums">{hasData ? fmt1(data.npk) : "–"}</span><span className="text-xs text-white/60">/100</span></Tile>
