...
import { CONDITION_LABEL, STATUS_LABEL } from "@/components/tables/ga-assets-table";
import {
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="grid gap-1.5">
                ...
                <Select value={f.status} onValueChange={(v) => setF((p) => ({ ...p, status: v ?? p.status }))}>
                  <SelectTrigger><SelectValue>{(v: string) => STATUS_LABEL[v] ?? v}</SelectValue></SelectTrigger>
                  <SelectContent>
