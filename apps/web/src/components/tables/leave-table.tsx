  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-end gap-2">
        ...
        {(from || to) && (
          <span className="text-muted-foreground text-xs">{filtered.length} dari {leave.length} cuti</span>
        )}
      </div>
      <DataTable columns={columns} data={filtered} getKey={(l) => l.id} searchPlaceholder="Cari karyawan / jenis…" pageSize={25} />
    </div>
  );
}
