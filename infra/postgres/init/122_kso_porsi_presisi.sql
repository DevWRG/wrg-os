-- 122 — `porsi_kso` diekspos dengan presisi 12 desimal, bukan 4.
--
-- SATU BARIS BERUBAH dari 121: round(rev.porsi_kso, 4) -> round(..., 12). Sisa definisi
-- disalin apa adanya dari 121 secara mekanis (bukan ditulis ulang), supaya perubahan
-- perilaku 121 — porsi berbasis reagen untuk faskes yang satu sisinya nol tes — terbawa
-- utuh dan tak ada peluang salah salin.
--
-- ── MASALAH YANG DIPECAHKAN: SATU-SATUNYA PORSI YANG BISA DIBACA SUDAH DIBULATKAN ──
-- Aturan porsi hidup sebagai CTE di dalam kso_asset_produktivitas_v; yang keluar cuma
-- kolom `porsi_kso`, dan sejak 105 kolom itu di-round(...,4).
--
-- Akibatnya konsumen di luar SQL menghadapi pilihan yang dua-duanya salah:
--   (a) pakai kolom yang dibulatkan  -> selisih pembulatan. Terukur Rp 4.968 dari
--       Rp 134 juta pada daftar reagen (#998) — kecil, tapi membuat dua angka di SATU
--       dialog tidak sama dan memancing pertanyaan tanpa jawaban.
--   (b) hitung porsi sendiri         -> DUPLIKASI ATURAN. Inilah yang saya pilih di
--       #998 (porsi diturunkan dari total_tes_customer_seskema), dan 121 langsung
--       membuktikan kenapa itu keliru: 121 mengubah aturannya untuk faskes yang satu
--       sisinya nol tes, sementara perhitungan di TypeScript tidak ikut berubah.
--       Untuk NGUDI WALUYO WLINGI porsi akan TERBALIK PENUH — sisi PER_TEST yang
--       memakai seluruh Rp 675 jt reagen hemodialisa dapat 0, sisi lain dapat 1,0 —
--       persis kesalahan yang 121 perbaiki, muncul kembali di layar sebelahnya.
--
-- Dengan 12 desimal, (a) hilang: selisih pembulatan turun ke bawah satu rupiah pada
-- nilai miliaran, sehingga membaca kolom ini menjadi pilihan yang benar dan (b) tidak
-- lagi punya alasan untuk ada. Aturan porsi tetap SATU, di SQL.
--
-- BUKAN PERUBAHAN ANGKA REVENUE. Yang dibulatkan hanya kolom yang DILAPORKAN; revenue di
-- dalam view sejak dulu dihitung dengan porsi presisi penuh (`rev.porsi_kso`, bukan
-- kolomnya). Jadi revenue_netto_customer tidak bergeser sedikit pun — yang berubah hanya
-- ketelitian angka yang bisa dibaca dari luar.
--
-- KONSEKUENSI TAMPILAN: porsi_kso kini bisa berbunyi 0,933544156600. Pembulatan untuk
-- dibaca manusia adalah urusan lapisan tampilan, bukan lapisan data — dan penanda
-- "skema ganda" di UI sudah membulatkannya sendiri.
