-- 069 — SJ→BAST Closed-Loop Tracker (F42, SHIPPING): extend F12
...
COMMENT ON TABLE shipment_tracking IS
  'F12+F42 — Tracking Pengiriman (SHIPPING): status kirim per SJ + ETA dari jarak (km), state machine draft→dikirim→terima→bast, dipicu WA hashtag #KIRIM/#BAST (terima manual via web).';
