-- 107 — satukan aturan atribusi kategori→skema: view produktivitas BACA `kso_kategori_skema`.
--
-- 106 sudah memindahkan peta "kategori pengadaan mana yang boleh dihitung sebagai revenue
-- skema mana" ke tabel `kso_kategori_skema`, tapi hanya `kso_tren_bulanan_v` yang
-- memakainya. `kso_asset_produktivitas_v` masih menyalin daftar yang sama di CTE
-- `kategori_skema` — dan salinan itu sudah dibawa berulang lewat 098 → 100 → 101 → 102 →
-- 103 → 104 → 105. Migrasi ini menghapus salinan terakhir.
--
-- KENAPA DUPLIKASI INI BERBAHAYA, BUKAN SEKADAR TIDAK RAPI: kalau kelak satu kategori
-- ditambah/dicabut dan hanya satu sisi yang diubah, tidak ada error yang muncul. Halaman
-- Ringkasan (tren, dari tabel) dan tabel Produktivitas (dari CTE) akan menampilkan revenue
-- berbeda untuk faskes yang sama, dan tidak ada total yang gagal rekonsiliasi untuk
-- menandainya — persis kelas kesalahan "metric benar, konteks salah" yang sudah beberapa
-- kali hanya terlihat dari output prod. Sejak sekarang satu-satunya tempat mengubah aturan
-- adalah isi tabel `kso_kategori_skema`.
--
-- BUKAN PERUBAHAN ANGKA. Isi tabel di prod per 2026-08-18 sudah persis sama dengan CTE-nya:
--     PER_TEST    {KSO}
--     BELI_REAGEN {ECAT,KSO,PL,REGULAR,RUTIN}
-- Selebihnya definisi view disalin apa adanya dari 105 — hanya blok CTE `kategori_skema`
-- yang berubah. Verifikasi setelah apply: 524 baris, dan agregat Rp/tes tidak bergeser.
--
-- EFEK SAMPING YANG HARUS DIKETAHUI: sejak migrasi ini, MENGOSONGKAN `kso_kategori_skema`
-- membuat view ini ikut kosong tanpa error — bukan nol revenue, tapi nol baris. Karena itu
-- migrasi ini menolak jalan kalau tabelnya kosong, dan tabel itu jangan pernah di-TRUNCATE
-- tanpa mengisinya kembali di transaksi yang sama.

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM kso_kategori_skema) THEN
    RAISE EXCEPTION
      'kso_kategori_skema kosong — jalankan 106 dulu; tanpa isinya kso_asset_produktivitas_v akan nol baris';
  END IF;
END $$;
