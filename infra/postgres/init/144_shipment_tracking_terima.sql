...
COMMENT ON TABLE shipment_tracking IS
  'F12+F42 — Tracking Pengiriman (SHIPPING): status kirim per SJ, state machine draft→dikirim→terima→bast, dipicu WA hashtag #KIRIM/#BAST (terima manual via web). distance_km/eta_days dihitung otomatis dari foto ber-geotag #KIRIM/#BAST (lihat 140_shipment_tracking_geo.sql).';
